...
<br>
<form class="layui-form orgForm" >
    <input type="hidden" id="orgId" name="orgId" value="${organize.orgId}">
    <input type="hidden" id="createTime" name="createTime" value="<fmt:formatDate value="${organize.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>">
    <div class="layui-form-item">
        <label class="layui-form-label">机构名：</label>
        <div class="layui-input-inline">
            <input id="orgName" type="text" name="orgName" value="${organize.orgName}" autocomplete="off" class="layui-input" required lay-verify="required">
        </div>
    </div>

    <div class="layui-form-item">
        <label class="layui-form-label">管辖区域：</label>
        <div class="layui-input-block" id="proId">
...
<script>
    $(function () {
        var proId = new Array();
        <c:forEach var="pro" items="${proList}" varStatus="status" step="1">
            proId[${status.index}] = ${pro.proId}
        </c:forEach>
    ...
    /*$(function () {
        layui.use(['form', 'layer'], function () {
            ...
            form.on('submit(*)',function (data) {
                ...
                $.ajax('organize/update',{
                    type:'POST',
                    data:$('.orgForm').serialize(),  //整个表单的内容
                    dataType:'json',
                    async:false,//默认为true异步
                    success:function (data) {
                        console.log(data);
